...
open class SMSegment: UIView {
    ...
    internal func setupUIElements() {
        DispatchQueue.main.async(execute: {
            if let appearance = self.appearance {
                ...
                self.bottomLine.backgroundColor = appearance.bottomLineOffSelectColor
            }
    ...
    open override func layoutSubviews() {
        ...
        self.bottomLine.frame = CGRect(x: (self.frame.size.width - titleViewFrame.size.width)/2, y: self.frame.height - 2, width: titleViewFrame.width, height: 2)
    }
    ...
    internal func setSelected(_ selected: Bool) {
        ...
        if selected == true {
            DispatchQueue.main.async(execute: {
                ...
                self.bottomLine.backgroundColor = self.appearance?.bottomLineOnSelectColor
            })
        }
        else {
            DispatchQueue.main.async(execute: {
                ...
                self.imageView.image = self.offSelectionImage
                self.bottomLine.backgroundColor = self.appearance?.bottomLineOffSelectColor
            })
        }
    }
