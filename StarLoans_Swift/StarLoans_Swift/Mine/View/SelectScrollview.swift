...
class SelectScrollview:UIView{
    ...
    var arrTitle:[String]?
    var onSelectColor: UIColor = UIColor(red: 242/255, green: 56/255, blue: 61/255, alpha: 1)
    var offSelectColor: UIColor = UIColor(red: 51/255, green: 51/255, blue: 51/255, alpha: 1)
...
//             make!.size.mas_equalTo()(CGSize.init(width: self.screenWidth, height: self.bounds.size.height - 44))            })
            vc.view.snp.updateConstraints({ (make) in
                make.size.equalTo(CGSize(width: screenWidth, height: bounds.size.height - 44))
            })
        }
    }
...
//        }
    ...
    public func createtopScrollBtn() {
        ...
        for (index,title) in arrTitle.enumerated() {
            ...
            btn.setTitleColor(offSelectColor, for: .normal)
...
//            })
            btn.snp.makeConstraints({ (make) in
                if let tempBtn = tempBtn {
                    make.left.equalTo(tempBtn.snp.right).offset(10)
                }else {
...
//        }
        topAnotherScrollView.snp.updateConstraints { (make) in
            make.centerX.equalTo(topScrollView.snp.left).offset(btnCenterX)
            make.width.equalTo(btn.frame.width)
        }
...
//        }
        topAnotherScrollView.snp.makeConstraints { (make) in
            make.top.equalToSuperview()
            make.centerX.equalTo(topScrollView.snp.left).offset(widthFirst/2)
            make.width.equalTo(widthFirst)
            make.height.equalTo(44)
        }
        var tempBtn:UIButton?
        ...
        for title in arrTitle {
            let btn = UIButton()
            btn.setTitle(title, for: .normal)
            btn.setTitleColor(onSelectColor, for: .normal)
            btn.titleLabel?.font = UIFont.systemFont(ofSize: 13)
            btn.titleLabel?.textAlignment = .center
...
//            })
            btn.snp.makeConstraints({ (make) in
                if let tempBtn = tempBtn {
                    ...
                    make.left.equalTo(topScrollView.snp.left)
                }
                make.top.equalToSuperview()
                make.size.equalTo(CGSize(width: width, height: 44))
            })
    ...
    lazy var bottomView:UIView = {
        let view = UIView()
        view.backgroundColor = onSelectColor
        return view
    }()
...
//                    })
                    vc[currentPage].view.snp.makeConstraints({ (make) in
                        make.left.equalTo(CGFloat(currentPage)*screenWidth)
                        make.top.equalToSuperview()
                        make.size.equalTo(CGSize(width: screenWidth, height: bounds.size.height - 44))
                    })
                  return
...
//            }
            topAnotherScrollView.snp.updateConstraints({ (make) in
                make.centerX.equalTo(topScrollView.snp.left).offset(centerInfor(offsetX: scrollView.contentOffset.x))
                make.width.equalTo(widthInfor(offsetX: scrollView.contentOffset.x))
            })
